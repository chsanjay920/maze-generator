...
do {
    ...
    maze[maze.findIndex(e => e.x == nonvisitedNBCell.x && e.y == nonvisitedNBCell.y)].status = "visited";

    //update the current cell to choosen cell
    cell = maze[maze.findIndex(e => e.x == nonvisitedNBCell.x && e.y == nonvisitedNBCell.y)];

    //pushing element to stack
    maze_stack.push(cell);

} while (maze_stack.length !== 0 || remainingEle.length !== 0);
